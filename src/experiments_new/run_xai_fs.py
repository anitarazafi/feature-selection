...
def apply_shap(X_train, X_val, X_test, model, cfg, k, seed=42):
...
def apply_lime(X_train, X_val, X_test, y_train, model, cfg, k, seed=42):
...
def run_xai_fs(dataset_name, seed=42, k=None):
    print(f"\n{'='*60}")
    print(f"= Running XAI-Based Feature Selection for: {dataset_name} (seed={seed})")
    print(f"{'='*60}\n")
    ...
    print(f"Pre-training explainer model (XGBoost) for SHAP and LIME (seed={seed})...\n")
    explainer_model = deepcopy(list(MODELS.values())[1])
    ...
    if shap_cfg.get("enabled", False):
        ...
        selected_sets["shap"] = selected_features
        importance_scores["shap"] = feature_importance.values
        ...
        with open(selector_dir / f"shap_k{k}_seed{seed}_features.json", "w") as f:
            json.dump(selected_features, f)
        feature_importance.to_csv(
            selector_dir / f"shap_k{k}_seed{seed}_importance.csv", header=True
        )
    ...
    if lime_cfg.get("enabled", False):
        ...
        fs_time = time.time() - fs_start
        selected_sets["lime"] = selected_features
        # LIME feature_importance is already sorted, reindex to match feature_names order
        importance_scores["lime"] = feature_importance.reindex(feature_names).values
        ...
        selector_dir = results_dir / "selectors" / "xai"
        selector_dir.mkdir(parents=True, exist_ok=True)
        with open(selector_dir / f"lime_k{k}_seed{seed}_features.json", "w") as f:
            json.dump(selected_features, f)
        feature_importance.to_csv(
            selector_dir / f"lime_k{k}_seed{seed}_importance.csv", header=True
        )
    ...
    if selected_sets:
        ...
        for agg_name, feature_set in aggregated.items():
            ...
            fs_name = f"x_set_{agg_name}"

            print(f"\n{'─'*60}")
            print(f"X-Set ({agg_name}): {feature_set}")
            print(f"X-Set ({agg_name}) size: {n_features}")
            print(f"{'─'*60}\n")
            ...
            with open(selector_dir / f"x_set_{agg_name}_seed{seed}.json", "w") as f:
                json.dump(set_info, f, indent=2)

            X_tr_x = X_train[feature_set]
            X_v_x  = X_val[feature_set]
            X_te_x = X_test[feature_set]
    ...
    results_df.to_csv(tables_dir / f"xai_seed{seed}.csv", index=False)
    print(f"\nSaved results to {tables_dir / f'xai_seed{seed}.csv'}")
