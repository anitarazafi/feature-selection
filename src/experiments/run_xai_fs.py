...
def apply_permutation_importance(X_train, X_val, X_test, y_val, model, cfg, k, seed=42):
...
def generate_feature_selection_latex(selected_sets, x_set, output_path, caption, label):
...
def run_xai_fs(dataset_name, seed=42):
    print(f"\n{'='*60}")
    ...
    # Pre-train explainer model with this seed
    print(f"Pre-training explainer model (XGBoost) for SHAP, LIME, and PI (seed={seed})...\n")
    ...
    if eli5_cfg.get("enabled", False):
        ...
        selector_dir = results_dir / "selectors" / "xai"
        selector_dir.mkdir(parents=True, exist_ok=True)
        with open(selector_dir / f"pi_k{k}_seed{seed}_features.json", "w") as f:
            json.dump(selected_features, f)
        feature_importance.to_csv(
            selector_dir / f"pi_k{k}_seed{seed}_importance.csv", header=True
        )
    ...
    if selected_sets:
        ...
        with open(selector_dir / f"x_set_seed{seed}.json", "w") as f:
    ...
    generate_latex_table(
        results_df,
        output_path=tables_dir / f"xai_seed{seed}.tex",
        caption=caption,
    ...
    generate_feature_selection_latex(
        selected_sets, x_set,
        output_path=tables_dir / f"xai_features_seed{seed}.tex",
